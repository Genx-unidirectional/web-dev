...
const arr5: number[] = [1, 2, 4, 5];
let arr6: [number, boolean, string] = [12, true, "ganesh"];
...
function findLargestStr(arr: string[]) {
  let largestStr = "";
  for (let i of arr) {
...
function createUppercase(arr: string[]) {
...
function doubleAndReturnArgs(arr1: number[], ...arr2: number[]) {
...
function makeArrayChunk(arr: number[], size: number) {
  let newArr: number[][] = [];
  let i = 0;
  while (i < arr.length) {
